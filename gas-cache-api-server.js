const API_KEYS = {
  'prod_key_abc123': { name: 'Production', partition: 'prod', rateLimit: 1000 },
  'dev_key_xyz789': { name: 'Development', partition: 'dev', rateLimit: 100 },
  'test_key_def456': { name: 'Testing', partition: 'test', rateLimit: 50 }
};
...
function doPost(e) {
  ...
  const { action, key, value, ttl, partition } = parsed;
  ...
  switch(action) {
    case 'get':
      if (!key) {
        result = { ok: false, error: 'key required' };
        break;
      }
      const val = cache.get(partKey(key));
      result = { ok: true, data: val, hit: !!val };
      logData.keyCount = 1;
      logData.hit = !!val;
      break;
    ...
    case 'set':
      if (!key || value === undefined) {
        result = { ok: false, error: 'key and value required' };
        break;
      }
    ...
    case 'getAll':
      if (!Array.isArray(key)) {
        result = { ok: false, error: 'key must be array' };
        break;
      }
      ...
      for (const [pk, val] of Object.entries(values)) {
        ...
        if (reconResult.ok) {
          ...
          reconstructErrors.push({ key: origKey, error: reconResult.error });
        }
      }
    ...
    case 'putAll':
      if (typeof value !== 'object') {
        result = { ok: false, error: 'value must be object' };
        break;
      }
      ...
      for (const [k, v] of Object.entries(value)) {
        ...
        if (!chunks.ok) {
          chunkErrors.push({ key: k, error: chunks.error });
          continue;
        }
        ...
        if (chunks.data.length === 1) {
          ...
          pairs[partKey(k)] = JSON.stringify({ chunks: chunks.data.length });
    ...
    case 'clear':
      ...
      if (allKeys.length > 0) {
        try {
          ...
          result = { ok: true, keysCleared: allKeys.length };
          logData.keysCleared = allKeys.length;
        } catch (err) {
          result = { ok: false, error: err.message };
        }
      } else {
        result = { ok: true, keysCleared: 0 };
      }
      break;
...
function chunkValue(value) {
  try {
    ...
    return { ok: false, error: `Chunk failed: ${err.message}` };
  }
}
...
function writeChunked(cache, key, chunks, ttl) {
  try {
    if (chunks.length === 1) {
      ...
      const meta = { chunks: chunks.length };
...
function deleteChunked(cache, key) {
  try {
    ...
    if (!metaVal) {
      cache.remove(key);
      return { ok: true, chunksDeleted: 0 };
    }
    ...
    try {
      ...
      if (meta.chunks) {
        ...
        return { ok: true, chunksDeleted: meta.chunks };
      }
    } catch {
...
function reconstructValue(metaVal, key, cache, partition) {
  if (!metaVal) return { ok: true, data: null };
  
  try {
    const meta = JSON.parse(metaVal);
    if (!meta.chunks) return { ok: true, data: metaVal };
    
    const chunkKeys = Array.from({length: meta.chunks}, (_, i) => 
